...
`
  ...
  function handleEnumType(type) {
    const name = transformTypeName(type.name, namingConvention)
...
`
  ...
  function handleEnumTypeCore(type) {
    return `types.enumeration("${transformTypeName(type.name, namingConvention)}", [
      ${type.enumValues
        .map(
          enumV =>
  ...
  function handleObjectType(type) {
    ...
    const name = transformTypeName(type.name, namingConvention)
    const origName = type.name
    const flowerName = transformName(toFirstLower(name), namingConvention)
...
`
  ...
  function handleInterfaceOrUnionType(type) {
    ...
    const fileName = transformTypeName(type.name, namingConvention) + "ModelSelector"
    const {
      primitiveFields,
      nonPrimitiveFields,
      imports
    ...
    interfaceOrUnionType &&
      interfaceOrUnionType.ofTypes.forEach(t => {
        const toBeImported = [`${transformTypeName(t.name, namingConvention)}ModelSelector`]
        if (isUnion) toBeImported.push(`${toFirstLower(transformTypeName(t.name, namingConvention))}ModelPrimitives`)
        addImportToMap(
          imports,
          fileName,
          `${t.name}Model.base`,
  ...
  function resolveFieldsAndImports(
    type,
    currentModuleName = `${transformTypeName(currentType, namingConvention)}Model.base`
  ) {
    ...
    function handleFieldType(fieldName, fieldType, isNested = false) {
      ...
      switch (fieldType.kind) {
        ...
        case "ENUM":
          primitiveFields.push(fieldName)
          const enumType = transformTypeName(fieldType.name, namingConvention) + "Enum"
    ...
    function handleObjectFieldType(fieldName, fieldType, isNested) {
      ...
      // import the type
      const modelType = transformTypeName(fieldType.name, namingConvention) + "Model"

      console.log("fieldName, fieldType, currentType, modelType", [
        fieldName,
        fieldType.name,
        currentType,
        modelType
      ...
      const realType = `types.late(()${
        format === "ts" ? ": any" : ""
      } => ${transformTypeName(fieldType.name, namingConvention)}Model)`
      ...
      refs.push([
        fieldName,
        transformTypeName(fieldType.name, namingConvention),
        isNested
      ])
  ...
  function generateFragments(
    ...
    output += nonPrimitiveFields
      .map(([field, fieldName]) => {
        const selector = transformTypeName(`${fieldName}ModelSelector`, namingConvention)
        let p = `  ${field}(builder`
        p += ifTS(
...
${objectTypes
  .map(t => transformTypeName(t, namingConvention))    
  .map(
    t =>
      `\nimport { ${t}Model${ifTS(
        `, ${t}ModelType`
  ...
  .join("")}
${enumTypes
  .map(t => `\nimport { ${transformTypeName(t, namingConvention)} } from "./${transformTypeName(t, namingConvention)}Enum${importPostFix}"`)
  .join("")}
${ifTS(
  inputTypes
    .map(
      t =>
        `\nexport type ${transformTypeName(t.name, namingConvention)} = {\n${t.inputFields
...
type Refs = {
${rootTypes
  .map(t => `  ${transformRootName(t, namingConvention)}: ObservableMap<string, ${transformTypeName(t, namingConvention)}ModelType>`)
  .join(",\n")}
}\n\n`)}\
/**
...
export const RootStoreBase = ${ifTS("withTypedRefs<Refs>()(")}${
  ...
  .extend(configureStoreMixin([${objectTypes
    ...
    .join(", ")}], [${rootTypes.map(s => `'${transformTypeName(s, namingConvention)}'`).join(", ")}]))
  .props({
${rootTypes
  .map(
    t =>
      `    ${transformRootName(t, namingConvention)}: types.optional(types.map(types.late(()${ifTS(
        ": any"
      )} => ${transformTypeName(t, namingConvention)}Model)), {})`
...
${optPrefix("\n    // ", sanitizeComment(description))}
  ...
  function printTsType(field, name, canBeUndefined = true) {
    ...
    return `${name}${canBeUndefined ? "?" : ""}: ${transformTypeName(typeValue, namingConvention)}`
  }
...
function transformRootName(text, namingConvention) {
  console.log("transformRootName", text)
  if (namingConvention === "js") {
